...
class BlogCategory(db.Model):
    ...
    parent_id = db.Column(db.Integer, db.ForeignKey('blog_category.id'), index=True)
    parent = db.relationship(lambda: BlogCategory,
                             remote_side=id,
                             backref=db.backref('sub_categories', lazy='dynamic'))
...
class BlogPost(db.Model):
    ...
    category = db.relationship(lambda: BlogCategory, backref=db.backref('posts', lazy='dynamic'))
...
class BlogComment(db.Model):
    ...
    replied_to_id = db.Column(db.Integer, db.ForeignKey('blog_comment.id'), index=True)
    replied_to = db.relationship(lambda: BlogComment,
                                 remote_side=id,
                                 backref=db.backref('sub_comments', lazy='dynamic'))
